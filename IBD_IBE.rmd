...
phall <- read.genepop("Q:/Research/All_Projects_by_Species/Phacelia SPECIES/Phacelia_formosula/Phacelia formosula_abiotic/PhfoAllGenePop_13Dec2016.gen")
...
#genind to genpop
ph.genepop #no TC
phall.genepop #with TC

### without TC
Dgen <- dist.genpop(ph.genepop,method=2)
Dgeo <- spatialdist.noTC
ibd <- mantel.randtest(Dgen,Dgeo) #repeat this for a distance matrix of climate variables! climate envelope
ibd
plot(ibd)
...
# Figure out which ones are missing from phall to match locaiton per collection (phfo.gen has all collection points with TC)

## example from adegenet - genind object iwth a matrix of xy coordinates
#### Monmonier’s algorithm to define genetic boundaries
temp <- ph1$pop
levels(temp) <- c(3,5)
temp <- as.numeric(as.character(temp))
plot(ph1$other$xy,pch=temp,cex=1.5,xlab='x',ylab='y')
...
biodata <- data.frame(soils.unique,bio1.ph,bio19.ph,bio18.ph,bio17.ph,bio16.ph,bio15.ph,bio14.ph,
                      bio13.ph,bio12.ph,bio11.ph,bio10.ph,bio9.ph,bio8.ph,bio7.ph,bio6.ph,
...
bio.dist <- dist(biodata[,30:48], method="manhattan") 

bio.prcomp <- prcomp(biodata[,30:48], center = TRUE)
print(bio.prcomp)
summary(bio.prcomp)
...
exclude <- paste(data$Var2[abs(data$Freq)>0.70],sep=",")

bionames <- names(biodata[,30:48])
keep <- bionames[!bionames %in% unique(exclude)]
...
ggplot(plot.bio, aes(bio1.ph, bio11.ph, colour = Species))+
  geom_point()+
  labs(x = "Annual mean temperature", y = "Mean temperature of coldest quarter")+
...
ggplot(plot.bio, aes(MUSYM, bio6.ph, colour = Species))+
  geom_violin()+
  geom_point()+
  labs(x= "Soil Type", y = "Min Temperature of Coldest Month")+
  theme_bw()
