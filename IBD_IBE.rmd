...
library(reshape2)
dgeo.df <- data.matrix(Dgeo)
dgeo.df <- data.frame(dgeo.df)
names(dgeo.df)<-phall@pop
dgeo.df$Pair <- phall@pop
dgeo.melt <- melt(dgeo.df)

ggplot(dgeo.melt, aes(x=variable,y=value))+
  geom_boxplot(aes(fill=variable))

plot(Dgeo,Dgenind, col=as.factor(phall@pop))
