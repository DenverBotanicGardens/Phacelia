...
for(rep in 1:10){
      ...
      LarimerMaxEnt <- maxent(x = rasterstack, p = phfo.prbg[phfo.prbg$County == "Larimer",c("lon","lat")],
                              a = phfo.prbg[phfo.prbg$County == "background", c("lon","lat")]) #, 
                              # path = "C:/Users/deprengm/Dropbox/Denver Botanic Gardens Postdoctoral Genetic Files/Phacelia project/larimer")
      
      JacksonMaxEnt <- maxent(x = rasterstack, p = phfo.prbg[phfo.prbg$County == "Jackson",c("lon","lat")],
                              a = phfo.prbg[phfo.prbg$County == "background", c("lon","lat")]) # , 
                              # path = paste("C:/Users/deprengm/Dropbox/Denver Botanic Gardens Postdoctoral Genetic Files/Phacelia project/jackson", sep=""))
      ...
      #register parallel computing backend
              ...
              foreach(i=unique(split), .combine=c)%dopar%{
                ...
                sub = raster::crop(rasterstack,raster::extent(rasterstack, min(rows_sub), max(rows_sub), 
                                                              1, ncol(rasterstack)))
                raster::predict(sub, LarimerMaxEnt, filename=paste(outnameL,"LarimerRep",rep,"slice",i,sep=""), overwrite=TRUE)
                raster::predict(sub, JAcksonMaxEnt, filename=paste(outnameJ,"JacksonRep",rep,"slice",i,sep=""), overwrite=TRUE)
              }
...
lapply(c("Jackson","Larimer"), function(i){
  ...
  lapply(1:10, function(k){
    patternmatch <- paste("Predict",i,k,i,"Rep", sep="")
...
hist(overlapsLarimer_Jackson)

Equivalency <- nicheEquivalency()
