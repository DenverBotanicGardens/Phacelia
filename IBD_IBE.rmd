...
plot(bgd)
plot(phfo.spdf[,c("lon","lat")])

#Extract soil layers to points data
soil<-list()
for(i in length(shape)){
  soil[i] <- extract(shape[[i]], phfo.spdf[,c("lon","lat")])
}
...
bioclimppoints <- data.frame(do.call(cbind,bio))
names(bioclimppoints) <- paste("bio",1:19,sep="")
biodata <- data.frame(phfo.UTM$Code, phfo.UTM$lat,phfo.UTM$lon,bioclimppoints)
...
library(car)
scatterplotMatrix(~bio6+bio19+bio12+bio11, 
                 diagonal = "histogram",
                 groups = biodata$phfo.UTM.Code,
                  data=biodata,
                 reg.line=FALSE,
                 smoother="")
