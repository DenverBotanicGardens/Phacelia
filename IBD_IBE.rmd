...
##START HERE!
# By group becuase hisorty and soil types might influence (Meirmans et. al. XXXX)
Dgeo_Jackson <-dist(phall$other$xy[phall@pop %in% c("")])
...
plot(density(Dgeo), xlim=c(-11,11), main = "Distance of geo, red = log transformed")
lines(density(log(Dgeo)),col="red")
...
plot(Dgeo,Dgenind, col=colslabels,pch=as.numeric(colslabels), cex=0.5) #the lower triangle by lower triangle
text(Dgeo,jitter(rep(1,length(Dgenind)),factor=5), 
...
Dgeo_JacksonandLarimer <- dist(phJacksonandLarimer$other$xy)
...
bio2 out  
```{r}
names(phfo.bioclim.soils[,c(21,27,29,32)])
cor_matrix <- cor(phfo.bioclim.soils[,c(21,27,29,32)])
corrplot(cor_matrix, method="square", type="upper")

forPCA <- phfo.bioclim.soils[,c(21,27,29,32)]
```
...
#     col="red",
#     pch=as.numeric((pcaphfo.bioclim.soils$Code[pcaphfo.bioclim.soils$FID==1])),
     pch = 1)
points(pcaphfo.bioclim.soils$Comp.1[pcaphfo.bioclim.soils$FID==2],
     pcaphfo.bioclim.soils$Comp.2[pcaphfo.bioclim.soils$FID==2], 
     #col="blue", #pch=as.numeric(pcaphfo.bioclim.soils$Code[pcaphfo.bioclim.soils$FID==2]))
     pch=2)
points(pcaphfo.bioclim.soils$Comp.1[pcaphfo.bioclim.soils$FID==3],
     ...
     pch=3)
legend("bottomright",
       #pch=c(as.numeric(pcaphfo.bioclim.soils$Code[pcaphfo.bioclim.soils$FID==1]),as.numeric(pcaphfo.bioclim.soils$Code[pcaphfo.bioclim.soils$FID==2]),16),col=c(rep("red",11),rep("blue",3),"goldenrod"),
       pch=c(1,2,3),
 #      col=c("red","blue","goldenrod"),
       legend = c("Jackson","Larimer","Grand")) 
#text(Comp.2~Comp.1, data=pcaphfo.bioclim.soils[pcaphfo.bioclim.soils$FID<4,], labels = pcaphfo.bioclim.soils$Code[pcaphfo.bioclim.soils$FID<4],cex=0.75)
