...
plot(ph1$other$xy,pch=temp,cex=1.5,xlab='x',ylab='y',
     col=temp)
legend("bottomright",leg=unique(ph1$pop)) 
...
boxplot(missing_data["Total",-13], horizontal=TRUE, xlab="Locus", ylab="Percent Missing")
stripchart(missing_data["Total",-13][i],method="jitter")
 rug(missing_data["Total",-13][i])
```
...
plot(bioclim.phfo[[1]])
points(phall[,c("lon","lat")])

bio<-list()
for(i in 1:19){
  bio[[i]] <- extract(bioclim.phfo[[i]], ph.dist[,c("Easting.1","Northing.1")])
}
filenames <- list.files("Q:/Research/All_Projects_by_Species/aa_Shapefiles_Maps/Phacelia formosula/SoilLayers")
flnms <- unique(sapply(strsplit(filenames,"[.]"),'[',1))
